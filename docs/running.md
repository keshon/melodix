...
| `VOICE_READY_DELAY_MS`    | Delay after joining a voice channel before sending Opus (avoids an OP4 race). | `500`         | Advertise end-to-end voice encryption. Set `false` only if the bot cannot join voice at all. | `true` |
