...
| `MAX_AUDIO_BITRATE`       | Cap on the YouTube audio format the native parser picks, in bits per second. Defaults to what a Discord voice channel actually carries — 64 kbps unless the guild is boosted. Uncapped, the parser can pick a 186 kbps format and forward it untouched, and end-to-end encryption means Discord cannot transcode it down: a desktop absorbs that, a phone on mobile data drops packets and the audio fades out and snaps back in a loop. Raise it to your channel's bitrate on a boosted guild (96k/128k/256k/384k by tier); `0` takes the best on offer. | `64000` |
