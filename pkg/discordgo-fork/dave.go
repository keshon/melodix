...
func (d *DAVESession) EncryptFrame(opusData []byte) ([]byte, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.encryptFrameLocked(opusData)
}

// TryEncryptFrame atomically checks whether DAVE is active and encrypts the
// frame in a single lock acquisition, eliminating the TOCTOU race between
// IsActive() and EncryptFrame(). Returns the original data unchanged if DAVE
// is not active.
func (d *DAVESession) TryEncryptFrame(opusData []byte) ([]byte, bool, error) {
	d.mu.Lock()
	defer d.mu.Unlock()

	if !d.active || d.frameCipher == nil {
		return opusData, false, nil
	}

	encrypted, err := d.encryptFrameLocked(opusData)
	if err != nil {
		return opusData, false, err
	}
	return encrypted, true, nil
}

func (d *DAVESession) encryptFrameLocked(opusData []byte) ([]byte, error) {
...
func (d *DAVESession) ActivateAfterWelcome() {
	d.mu.Lock()
	defer d.mu.Unlock()
	d.hasPendingKey = false
	if d.senderKey != nil {
