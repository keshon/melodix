...
func (v *VoiceConnection) opusSender(ctx context.Context, rate, size int) {
	...
	for i := uint32(0); true; i++ {
		...
		v.Cond.L.Unlock()
		if dave != nil {
			if encrypted, ok, err := dave.TryEncryptFrame(recvbuf); err != nil {
...
func (v *VoiceConnection) handleDAVEBinary(message []byte) {
	...
	case 30:
		...
		dave.ActivateAfterWelcome()
		dave.HandlePrepareTransition(transitionID, 1)
		v.log(LogInformational, "DAVE encryption prepared and ACTIVE after Welcome")
