...
func TestRecovery_CacheHit_ServesBlobAndSkipsParser(t *testing.T) {
	...
	if opened := rs.track.Clone(); !opened.Cached || rs.curParser != "" {
		t.Fatalf("cache hit should set Cached and empty parser (Cached=%v parser=%q)", opened.Cached, rs.curParser)
	}
...
func TestRecovery_CacheInstantFail_FallsBackToParser(t *testing.T) {
	...
	if opened := rs.track.Clone(); opened.Cached || opened.CurrentParser != "p1" {
