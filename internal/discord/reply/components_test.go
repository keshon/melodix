...
func TestButtonsKeepTheirCustomID(t *testing.T) {
	rows := DiscordComponents([]cmdadapter.ActionRow{{Buttons: []cmdadapter.Button{
...
func TestButtonStyleDefaultsToSecondary(t *testing.T) {
	cases := map[cmdadapter.ButtonStyle]discordgo.ButtonStyle{
		cmdadapter.SecondaryButton: discordgo.SecondaryButton,
		cmdadapter.PrimaryButton:   discordgo.PrimaryButton,
		cmdadapter.SuccessButton:   discordgo.SuccessButton,
		cmdadapter.DangerButton:    discordgo.DangerButton,
	}
