...
// SlashRunner is optional: when implemented, slash invocations use typed *SlashInteractionContext.
type SlashRunner interface {
	RunSlash(ctx *SlashInteractionContext) error
}
...
func (a *DiscordAdapter) Run(ctx context.Context, inv *commandkit.Invocation) error {
	if sr, ok := a.Cmd.(SlashRunner); ok {
		if si, ok := inv.Data.(*SlashInteractionContext); ok {
			return sr.RunSlash(si)
		}
	}
